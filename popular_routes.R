...
no_log_count <- all_data %>%
  group_by(Route) %>%
  summarize(count = sum(Attempted)) %>%
  arrange(desc(count)) %>%
  ggplot(aes(x = reorder(Route, - count), y = count, fill = Route))+
  geom_bar(stat = "identity")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  guides(fill = FALSE) +
  labs(x = "Route to the Summit", y = "Count of Hikers")

ensure_directory("figures")
ggsave("figures/popular_routes.png", no_log_count)